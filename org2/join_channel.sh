...
fetchChannelBlock() {
  setGlobalsForPeer0Org2
  peer channel fetch 0 $CHANNEL_NAME.block -o localhost:7050 --ordererTLSHostnameOverride orderer0.amit.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA
}
joinChannel() {
  setGlobalsForPeer0Org2
...
updateAnchorPeers() {
  ...
  setGlobalsForPeer0Org2
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer0.amit.com -c $CHANNEL_NAME -f ../org2/${CORE_PEER_LOCALMSPID}anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA
